    ...
    if (req.method === 'GET') {
      ...
      try {
        ...
        const posts = feed.items.map((item) => {
          ...
          // Try contentSnippet first (auto-generated by rss-parser)
          if (item.contentSnippet) {
            excerpt = item.contentSnippet;
          }
          // Try content:encoded (common in Medium/WordPress)
          else if (item['content:encoded']) {
            const content = item['content:encoded'];
            // Strip HTML tags
            excerpt = content.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
          }
          // Try content field
          else if (item.content) {
            excerpt = item.content.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
          }
          // Try description
          else if (item.description) {
            excerpt = item.description.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
          }
          // Try summary
          else if (item.summary) {
            excerpt = item.summary.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
          }
